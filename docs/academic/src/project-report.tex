...
Name               & : & \hrulefill \\[0.7em]
Register Number    & : & \hrulefill \\[0.7em]
...
FR-3  & Correct metadata & Allow a wrong title to be corrected in place, and
        propagate the correction to every stored passage. \\
...
\subsection{The guard that tested for silence}
...
\begin{itemize}[leftmargin=*]
  \item Figure: Library, the collection, its folio, the panels reporting
        models in use and drafts in progress, and the shelf.
  \item Figure: LitGraph, the map, with themes as regions and gap markers.
  \item Figure: Scribe, the file tree, the editor and the compiled PDF.
  \item Figure, Bench, what the machine can run and which model serves which
        task.
\end{itemize}

\noindent\emph{[Screenshots to be inserted from the running application before
final submission.]}
...
T-11 & Rename propagates to every chunk & All chunks updated & Pass \\
T-12 & Rename with a blank title & Rejected, 400 & Pass \\
T-13 & Rename an absent document & 404, nothing written & Pass \\
...
\bottomrule
\caption{Extraction accuracy, curated versus randomly sampled papers}
...
\midrule
\endhead
Backend (pytest)      & 984 & 984 & \textasciitilde13\,s \\
Interface (vitest)    & 198 & 198 & \textasciitilde5\,s \\
Contract              & 5   & 5   & $<$1\,s \\
Route reachability    & 16  & 16  & $<$2\,s \\
\bottomrule
\caption{Suite results at the time of writing}
\label{tab:suites}
\end{longtable}
...
Three defects shared one shape, described in Section~4.3.3, and it is the
